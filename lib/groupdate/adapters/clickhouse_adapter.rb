  ...
  module Adapters
    class ClickhouseAdapter < BaseAdapter
      def group_clause
        ...
        day_start_column = "toTimeZone(#{column}, ?) - INTERVAL ?"
        ...
        query =
          ...
          when :custom
            ["TO_TIMESTAMP(FLOOR(EXTRACT(EPOCH FROM #{column}) / ?) * ?)", n_seconds, n_seconds]
          else
            if day_start == 0
              # prettier
              ["toTimeZone(DATE_TRUNC(?, #{day_start_column}), ?)", period, time_zone, day_start_interval, time_zone]
            else
              # ["(DATE_TRUNC(?, #{day_start_column}) + INTERVAL ?) AT TIME ZONE ?", period, time_zone, day_start_interval, day_start_interval, time_zone]
              ["toTimeZone(DATE_TRUNC(?, #{day_start_column}) + INTERVAL ?, ?)", period, time_zone, day_start_interval, day_start_interval, time_zone]
            end
          end
